import java.util.Arrays;
import java.util.Scanner;

class Edge implements Comparable<Edge> {
    int src = 0;
    int dst = 0;
    int weight = 0;

    // when we will call Arrays.sort(edge), it will sort the array based on their weight
    public int compareTo(Edge compareEdge) {
        return this.weight - compareEdge.weight;
    }
}

public class kruskalsMST {

    private static int find(int v, int[] parent) {
        if (parent[v] == v)
            return v;
        return find(parent[v], parent);
    }

    private static void krushkals(Edge[] inp, int v, int e) {
        // sorting
        Arrays.sort(inp);
        Edge op[] = new Edge[v - 1];
        int k = 0;
        int parent[] = new int[v];
        for (int i = 0; i < v; i++)
            parent[i] = i;

        for (int i = 0; i < e; i++) {
            if (k == v - 1)
                break;

            Edge currEdge = inp[i];
            int src_parent = find(currEdge.src, parent);
            int dst_parent = find(currEdge.dst, parent);

            if (src_parent != dst_parent) {
                op[k] = currEdge;
                k++;
                parent[src_parent] = dst_parent;
            }
        }

        for (int i = 0; i < op.length; i++)
            System.out.println(op[i].src + " ->" + op[i].dst + " ->" + op[i].weight);
    }

    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);

        System.out.print("Enter the number of nodes and edges you want in the graph: ");
        int v = sc.nextInt();
        int e = sc.nextInt();

        Edge inp[] = new Edge[e];

        System.out.print("Enter the src node, destination node and weight of their respective edge: ");
        for (int i = 0; i < e; i++) {
            inp[i] = new Edge();
            inp[i].src = sc.nextInt();
            inp[i].dst = sc.nextInt();
            inp[i].weight = sc.nextInt();
        }

        krushkals(inp, v, e);
        sc.close();
    }
}